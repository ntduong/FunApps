...
def main():
    ...
    while True:
        ...
        playerSymbol, computerSymbol = select_symbol()
        showHints = False
        turn = who_goes_first()
        ...
        while True:
            if turn == 'player':
                if showHints:
                    validMovesBoard = get_board_with_valid_moves(mainBoard, playerSymbol)
                    draw_board(validMovesBoard)
                else:
                    draw_board(mainBoard)
                show_points(mainBoard, playerSymbol, computerSymbol)
                
                move = get_player_move(mainBoard, playerSymbol)
                if move == 'quit':
                    print 'Thanks for playing!'
                    sys.exit()
                elif move == 'hints': # toggle hint mode
                    showHints = not showHints
                    continue
                else:
                    make_move(mainBoard, playerSymbol, move[0], move[1])
                if get_valid_moves(mainBoard, computerSymbol) == []: # can't make a move
                    break
                else:
                    turn = 'computer'
                ...
                r,c = get_computer_move(mainBoard, computerSymbol)
                make_move(mainBoard, computerSymbol, r, c)
                if get_valid_moves(mainBoard, playerSymbol) == []:
                    break
                else:
                    turn = 'player'
        ...
        if scores[playerSymbol] > scores[computerSymbol]:
            print 'You win by %s points!' %(scores[playerSymbol]-scores[computerSymbol])
        elif scores[playerSymbol] < scores[computerSymbol]:
            print 'You lost by %s points!' %(scores[computerSymbol]-scores[playerSymbol])
        else:
